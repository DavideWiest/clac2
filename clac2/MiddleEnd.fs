...
let checkManipulations manipulations functionSignatureMap : string option =
    ...
    let rec checkManipulation (m: Reference array) = 
        match m[0] with
        | Primitive _ -> if m.Length = 1 then None else Some ("Primitive treated as function: " + string m[0])
        | Fn f ->
            ...
            | Primitive p -> Some (sprintf "Argument count mismatch of preceding function %s. Primitive %A cannot be treated as function. " f p)
            | Fn f' -> 
